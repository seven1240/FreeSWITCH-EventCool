...
-export([send/2, api/3, api/2, bgapi/3, bgapi/4, event/2,
		...
		get_event_header/2, get_event_header/3, get_event_body/1,
		get_event_name/1, getpid/1, sendmsg/3, sendmsg/4,
...
get_event_header(Headers, Needle, DefaultVal) ->
	case get_event_header(Headers, Needle) of
			...
			{ok, JobID} ->
				...
				receive % wait for the job's reply
					{bgok, JobID, Reply} ->
						...
						io:format("freeswitch reply~p~n", [Reply]),
						Fun(ok, Reply);
					{bgerror, JobID, Reply} ->
						io:format("freeswitch reply~p~n", [Reply]),
						Fun(error, Reply)
...
sendmsg(Node, UUID, Application, Args) ->
	sendmsg(Node, UUID, [{"call-command", "execute"}, {"execute-app-name", Application}, {"execute-app-arg", Args}]).
